...
App.notAKeypress = function (e) {
  ...
  if (e.which == App.keyCodes.BACKSPACE) { return true; }
...
App.setPreventDefaultForKey = function (e) {
  ...
  if (e.which == App.keyCodes.BACKSPACE)       { e.preventDefault();   }

  // in firefox, single quote and forward slash do a "quick search"
  if (e.which == App.keyCodes.SINGLE_QUOTE)    { e.preventDefault();   }
  if (e.which == App.keyCodes.FORWARD_SLASH)   { e.preventDefault();   }

  // space will page down if the page is long enough, so don't do that
  if (e.which == App.keyCodes.SPACE)           { e.preventDefault();   }

  // tab shouldn't take us out of the typing window
  if (e.which == App.keyCodes.TAB)             { e.preventDefault();   }
};
